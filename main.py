...
# --- 演出用GIF ---
# Takashiさんの好きな料理GIFのURLに書き換えてください
GIF_URL = "https://media.giphy.com/media/
v1.Y2lkPTc5MGI3NjExOHJid3R3bmZyZzR4bmZ4bmZ4bmZ4bmZ4bmZ4bmZ4bmZ4bmZ4JmVwPXYxX2ludGVybmFs
X2dpZl9ieV9pZCZjdD1n/l0HlUUxwYvSJy05JS/giphy.gif"
def create_qr(options):
return QuickReply(items=[QuickReplyButton(action=MessageAction(label=opt,
...
try:
line_bot_api.show_loading_animation(ShowLoadingAnimationRequest(chat_id=user_id,
loading_seconds=60))
except: pass
# 調理GIFを送信
try:
...
line_bot_api.reply_message(event.reply_token, TextSendMessage(
text="オーダー入りました！👨‍🍳\nただいまレンジで加熱中です。完成まで1分ほどお待ちくださ
い..."
))
# AI生成
prompt = f"家族構成と食材（{user_message}）に合わせ、元ラーメン店長の視点で、300文字以内
の簡潔なレシピとプロのコツを提案してください。"
response = model.generate_content(prompt)
# チン！の通知演出
line_bot_api.push_message(user_id, TextSendMessage(text="🔔 ピーッ！ピーッ！ ＼ チ
ン！ ／"))
# レシピ表示
line_bot_api.push_message(user_id, TextSendMessage(text=f"お待たせしました！本日の
特製メニューです✨\n\n{response.text}"))
return
